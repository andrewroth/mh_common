module Common::Core::Ca::ManualDonation
  def self.included(base)
    base.class_eval do

      def self.STATUSES() %w(pending received invalid) end
      def self.LIST_COLUMNS() %w(motivation_code created_at donor_name donation_type original_amount conversion_rate amount status) end
      def self.LIST_TYPES() { 'original_amount' => 'currency', 'amount' => 'currency' } end
